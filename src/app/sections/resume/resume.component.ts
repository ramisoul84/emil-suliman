...
export class ResumeComponent implements AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.mapHeight = this.gridService.getMaxHeight(7)
  ...
  private handleScroll(scrollY: number): void {
    ...
    const isInViewport = this.EndHeight >= this.timelineTop && this.EndHeight <= this.timelineBottom;
    ...
    if (isInViewport) {
      ...
      // Check if we need to jump to correct frame
      if (this.shouldJumpToFrame()) {
        this.jumpToCorrectFrame();
      } else {
        // Normal playback
  ...
  private shouldJumpToFrame(): boolean {
    if (!this.timelineItem || this.totalFrames === 0) return false;

    const viewportTop = this.scrollY;
    const viewportBottom = this.scrollY + window.innerHeight;

    // Calculate where the current frame should be displayed in the timeline
    const frameProgress = this.currentFrame / this.totalFrames;
    const framePositionInTimeline = this.timelineTop + (frameProgress * (this.timelineBottom - this.timelineTop));

    // If frame position is outside viewport (with some margin), we should jump
    const isFrameAboveViewport = framePositionInTimeline < (viewportTop - 100); // 100px buffer
    const isFrameBelowViewport = framePositionInTimeline > (viewportBottom + 100); // 100px buffer

    return isFrameAboveViewport || isFrameBelowViewport;
  }

  private jumpToCorrectFrame(): void {
    if (!this.timelineItem || !this.mapItem || this.totalFrames === 0) return;

    const viewportTop = this.scrollY;

    // Calculate what frame should be at the current scroll position
    const scrollProgress = (viewportTop - this.timelineTop) / (this.timelineBottom - this.timelineTop);
    const targetFrame = Math.floor(scrollProgress * this.totalFrames);

    // Ensure target frame is within bounds
    const clampedFrame = Math.max(0, Math.min(this.totalFrames, targetFrame));

    // Jump to the correct frame
    this.timelineItem.goToAndStop(clampedFrame, true);
    this.mapItem.goToAndStop(clampedFrame, true);
    this.currentFrame = clampedFrame;

    console.log(`Jumped to frame ${clampedFrame}`);

    // Continue playing from the correct position
    this.timelineItem.play();
    this.mapItem.play();
  }
